...
- **News banner** — the highlighted `.news` block at the top of the hero in `index.html`. Add/remove `<li>` items in `.news-list`; the small `.news-tag` label colors each item (cyan by default, violet with the `rev` class).
- **Reviewer sign-up** — `reviewers.html` hosts a form that emails each submission to the workshop inbox via [FormSubmit](https://formsubmit.co) (no backend needed on GitHub Pages). The form `action` is `https://formsubmit.co/info@worldmodels-physicalai.com`. **Activation:** the first time the form is submitted, FormSubmit sends a one-time confirmation email to that address — click the link in it once to start receiving submissions. To hide the address from the page source, submit once, then swap the `action` for the random `https://formsubmit.co/<hash>` endpoint FormSubmit provides. On success it redirects to `reviewers.html?sent=1`, which shows a thank-you message.
